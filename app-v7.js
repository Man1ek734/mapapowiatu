console.info("Mapa Powiatu Opatowskiego — maska SVG v7");
const CONFIG = {
...
// Niezależna maska SVG: cały ekran szary, a Powiat Opatowski wycięty jako przezroczyste okno.
const countyMaskSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
countyMaskSvg.setAttribute("aria-hidden", "true");
Object.assign(countyMaskSvg.style, {
  position: "absolute",
  inset: "0",
  width: "100%",
  height: "100%",
...
const countyMaskPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
countyMaskPath.setAttribute("fill", "#6d7270");
countyMaskPath.setAttribute("fill-opacity", "0.82");
countyMaskPath.setAttribute("fill-rule", "evenodd");
countyMaskSvg.appendChild(countyMaskPath);
map.getContainer().appendChild(countyMaskSvg);

function updateCountyMask() {
  if (!countyRings.length) {
    countyMaskPath.setAttribute("d", "");
    return;
  }

  const size = map.getSize();
  let d = `M0 0 H${size.x} V${size.y} H0 Z `;

  countyRings.forEach(ring => {
  ...
  countyMaskPath.setAttribute("d", d);
}
...
function drawBoundary(data) {
  ...
  const relation = (data.elements || []).find(
    element => element.type === "relation" && element.id === 2635571
  );
  if (!relation) return;

  const rings = stitchBoundaryRings(relation);
  if (!rings.length) {
    console.error("Nie udało się złożyć granicy relacji 2635571.");
    return;
  }
...
async function loadData() {
  ...
  try {
    const places = await overpass(placesQuery);
    const boundary = places;
...
map.on("move zoom resize viewreset", updateCountyMask);
